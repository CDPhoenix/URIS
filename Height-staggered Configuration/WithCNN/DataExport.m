...
function [dataset,model] = DataExport(modelFilename,datafilename,datatag,plotgroup)%range_txt_x,range_txt_y,X,Y,exper)

    %[range_txt,range_numerical] = range_settle(range_txt_x,range_txt_y,X,Y);

    [dataset,model] = DataProcess(modelFilename,datafilename,datatag,plotgroup);%exper,range_txt);
