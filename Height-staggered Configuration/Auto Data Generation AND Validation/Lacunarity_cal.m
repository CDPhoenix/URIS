classdef Lacunarity_cal
    methods(Static)
        function [Lsc,H] = calculation(model,modelname,export)
            %B = model.geom('geom1').feature('blk2').getDouble('z'); 
            % Ground-to-panel height [m]
            B1 = model.geom('geom1').feature('blk2').getDoubleArray('pos');
            B2 = model.geom('geom1').feature('blk3').getDoubleArray('pos');
            B3 = model.geom('geom1').feature('blk4').getDoubleArray('pos');
            ...
            [H1,~,~,~] = mphevaluate(model, 'Panel_height1');
            [H2,~,~,~] = mphevaluate(model, 'Panel_height2');
            [H3,~,~,~] = mphevaluate(model, 'Panel_height3');
            ...
            Depth = mphevaluate(model,'Panel_spanwise');
            ...
            row_num = 10;
            resolution = thick*0.5;%0.001    % Spatial resolution of created matrix [m] (pref.: res <= 0.5*L_{minimumFeatureLength})
            
            %B1 = B1*100;
            %B2 = B2*100;
            %B3 = B3*100;
            modelname = [modelname,'Array'];
